...
const translateJsonButton = document.querySelector(
  ".editor__actions--translate"
);
const languageInput = document.querySelector(".form__input--language");
const setLanguageButton = document.querySelector(
  ".editor__actions--set-language"
);
  ...
  const json = [
    {
      type: "div",
      value: "Hello World",
      attributes: { hello: "world", src: "test" },
      children: [
        { type: "h1", value: "test" },
        {
          type: "p",
...
const translateJson = () => {
  const { elements, translations } = nodeTranslator(
    getJsonPanelContent(),
    getLanguage()
  );
  ...
  setPreviewPanelContent(
    translationParser(elements, translations, getLanguage())
  );
};
...
setLanguageButton.onclick = () => {
  ...
  setPreviewPanelContent(
    translationParser(getJsonPanelContent(), translations, getLanguage())
  );
};
